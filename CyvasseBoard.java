...
public class CyvasseBoard extends CreateBoard {
    ...
    //checks if the person moving is the person's actual turn
...
//   checks if piece is inherently stronger than another
...
//checks if surrounding pieces is stronger than the individual pieces in order to take it
    public boolean strongerthan(int x1, int y1, int x2, int y2, int player) {
        int totalws = 0;
        int totalbs = 0;
        if ((x1 > 0 && x1 < 9) && (x2 > 0 && x2 < 9) && (y1 > 0 && y1 < 9) && (y2 > 0 && y2 < 9)) {
            if (!Pieces[x2][y2].equals(Nothing)) {
            ...
            } else if (Pieces[x2][y2].equals(Nothing)) {
                return true;
            }
